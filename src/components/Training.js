...
function Training() {
  ...
  const trainingData = getTrainingData(option);
              ...
              {trainingData.topics.map((topic, index) => (
                  ...
                  <div className="training-card p-4 mb-3" key={index}>
                    ...
                    {topic.description.map((description, index) => (
                      <p className="" key={index}><i className="fa-regular fa-hand-point-right"></i> {description}</p>
                    ))}
                    <div className="p-2 activity">
                    Activity/Scenario: {topic.activity? topic.activity:topic.scenario ||"No Activity"}
                    
                    </div>
                    <div>
                      {topic.conclusion?<p className=' mt-4 fw-bolder text-secondary text-center'>{topic.conclusion}</p>:""}
